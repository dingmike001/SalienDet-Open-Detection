...
class SingleStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        self.used = json.load(open(test_cfg['used_path']))
    ...
    def saliency_to_img(self, img_metas, imgs, json_path, mode, bias):
        ...
        for i in range(img_num):
            ...
            pad_w = img_meta['pad_shape'][1]
            pad_h = img_meta['pad_shape'][0]
            w_ratio = ori_w / new_w
            h_ratio = ori_h / new_h
            pic_name = img_meta['ori_filename']
            if mode < 2:
                img_info = all_data[pic_name]
                bboxs_info = img_info['data']
            else:
            ...
            # saliency_transformer = cv2.saliency.StaticSaliencyFineGrained_create()
            index = img < 800
            img[index] = 0
            ...
            if suc:
                ...
                img_sal[img_idx] = 0
                img_tmp = np.zeros((pad_h, pad_w), dtype=float)
                img_tmp[:new_h, :new_w] = img_sal
                img_sal = torch.tensor(img_tmp, dtype=torch.float, device=device)
                # img_sal = img_sal + torch.ones_like(img_sal)
                img_fast_new = torch.ones_like(img_fast)
                for kkk in range(len(img_fast)):
                    img_fast_sub = img_fast[kkk]
                    # img_fast_sub_n = img_fast_sub*img_sal
                    img_fast_sub_n = img_fast_sub + img_sal
                    img_fast_new[kkk, :, :] = img_fast_sub_n
